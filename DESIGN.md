...
| | `--pr` | `--branch` |
|---|---|---|
| Base branch from | `gh pr view` → `baseRefName` | `--branch BASE`, else `gate.conf`'s `base_branch=`, else `main` |
...
`execution=readonly` would launch its agents with full trusted Bash. The diff still uses
